export const PRODUCT_SETTINGS = {
  storeName: "HomeCarShop",
  name: "Scratch Repair",
  sku: "11-SCRATCHREPAIR",
  currency: "KM",
  shippingCost: 13,
  pixelId: "1016213196582153",
    country: 'BA',
};

export const BUNDLES = [
  {
    id: 1,
    name: "Start Paket",
    qty: 1,
    price: 35,
    oldPrice: 60,
    savings: "50%",
    description: "1 x Scratch repair (50ml)",
    freeShipping: false
  },
  {
    id: 2,
    name: "Pro Paket",
    qty: 2,
    price: 59,
    oldPrice: 120,
    savings: "15 KM EXTRA",
    label: "NAJPRODAVANIJE",
    popular: true,
    description: "2 x Scratch repair",
    freeShipping: false
  },
  {
    id: 3,
    name: "Mega Paket",
    qty: 3,
    price: 79,
    oldPrice: 180,
    savings: "MAX UŠTEDA",
    label: "BEST BUY",
    description: "3 x Scratch repair",
    freeShipping: true
  }
];

export const REVIEWS = [
  {
    name: "Emir Hadžić",
    city: "Sarajevo",
    text: "Nisam vjerovao dok nisam probao. Imao sam ružnu ogrebotinu na vratima od parkinga. Naprskao sam, obrisao i nestala je! Svaka čast.",
    rating: 5,
    image: "https://i.pravatar.cc/150?img=11"
  },
  {
    name: "Siniša Vuković",
    city: "Banja Luka",
    text: "Dostava je bila stvarno brza, stiglo je za tri dana. Sprej radi tačno ono što piše. Preporuka svima koji čuvaju auto.",
    rating: 5,
    image: "https://i.pravatar.cc/150?img=3"
  },
  {
    name: "Martina Zovko",
    city: "Mostar",
    text: "Koristila sam na crnom autu, strah me bilo da se ne vidi razlika jer je univerzalan. Savršeno se stopilo. Odlična stvar!",
    rating: 5,
    image: "https://i.pravatar.cc/150?img=5"
  },
  {
    name: "Jasmin Delić",
    city: "Tuzla",
    text: "Kupio sam dva pakovanja. Mnogo lakše se koristi od onih pasti što se trljaju satima. Samo naprskaš i obrišeš.",
    rating: 4,
    image: "https://i.pravatar.cc/150?img=60"
  }
];
